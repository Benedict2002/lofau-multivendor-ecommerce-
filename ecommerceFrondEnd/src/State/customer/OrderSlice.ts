...
import type { Order, OrderItem, OrderState } from "../../types/orderType";
import { api } from "../../config/Api";
import type { Address } from "../../types/UserType";
...
>(
  ...
  async ({ paymentId, jwt, paymentLinkId }, { rejectWithValue }) => {
    console.log("🔥 paymentSuccess thunk CALLED", {
      paymentId,
      jwt,
      paymentLinkId,
    });
...
export const cancelOrder= createAsyncThunk<Order, any>(
    ...
    async(orderId,{rejectWithValue}) =>{
        try{
            ...
            return rejectWithValue(
                "An error occured while cancelling the order."
            )
        }
    }
)
...
const orderSlice = createSlice({
    ...
    extraReducers: (builder: any) =>{
        builder
        .addCase(fetchUserOrderHistory.pending, (state: any) =>{
            state.loading = true;
            state.error= null;
            state.orderCanceled=false;
        })
        .addCase(fetchUserOrderHistory.fulfilled, (state: any, action :PayloadAction<Order[]>) =>{
            state.orders = action.payload;
            state.loading= false;
            
        })
        .addCase(fetchUserOrderHistory.rejected, (state: any, action :any) =>{
            state.loading = false;
            state.error= action.payload as string;
            
        })

        .addCase(fetchOrderById.pending, (state: any) =>{
            state.loading = true;
            state.error= null;
            
        })
        .addCase(fetchOrderById.fulfilled, (state: any, action :PayloadAction<Order>) =>{
            state.currentOrder = action.payload;
            state.loading= false;
            
        })
        .addCase(fetchOrderById.rejected, (state: any, action :any) =>{
            state.loading = false;
            state.error= action.payload as string;
            
        })
        .addCase(createOrder.pending, (state: any) =>{
            state.loading = false;
            state.error= null; 
        })
        .addCase(createOrder.fulfilled, (state: any, action: PayloadAction< any>) =>{
        ...
        .addCase(fetchOrderItemById.pending, (state: any) =>{
            state.loading = true;
            state.error= null; 
        })
        .addCase(fetchOrderItemById.fulfilled, (state: any, action: any) =>{
            state.loading = false;
            state.orderItem= action.payload as string; 
        })
        .addCase(fetchOrderItemById.rejected, (state: any, action: any) =>{
            state.loading = false;
            state.error= action.payload as string; 
        })
        .addCase(paymentSuccess.pending, (state: any) =>{
            state.loading = true;
            state.error= null; 
        })
        .addCase(paymentSuccess.fulfilled, (state: any, action : any) =>{
            state.loading = false;
            console.log('Payment successful :', action.payload) 
        })
        .addCase(paymentSuccess.rejected, (state: any, action:any) =>{
            state.loading = false;
            state.error= action.payload as string; 
        })
        .addCase(cancelOrder.pending, (state: any) =>{
            state.loading = true;
            state.error= null; 
            state.orderCanceled=false;
        })
